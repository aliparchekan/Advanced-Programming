#include "artist_page_handler.hpp"

using namespace std;

string artist_page_handler::parse_token(Request* req){
...
string artist_page_handler::prepare_error_html() {
...
string artist_page_handler::prepare_normal_html(const List& result) {
	if (result.size() == 0)
	{
...
void artist_page_handler::callback(Request* req, Response* res) {
	string token;
	try{
	...
	List result;
	result = main_data->search_all_artists();
